...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ApiErrorResponse> handleDomain(NotificationServiceException ex, HttpServletRequest req) {
        if (ex.getStatus().is5xxServerError()) {
            log.error("NotificationServiceException: {}", ex.getMessage(), ex);
        } else {
            log.info("NotificationServiceException: {}", ex.getMessage());
        }
